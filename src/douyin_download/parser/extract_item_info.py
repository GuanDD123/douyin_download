...
def _extract_common_info(item: dict, settings: Settings) -> dict[str, str]:
    ...
    if desc := _extract_value(item, 'desc'):
        result['desc'] = clear_spaces(
            filter_name(desc, settings.illegal_char))[:settings.file_description_max_length]
    else:
        result['desc'] = '作品描述为空'
...
def extract_item_info_list(item_list: list[dict], settings: Settings, account: Account) -> list[ItemInfo]:
    ...
    for item in item_list:
        ...
        if (result['create_time_date'] > account.latest_date) or (result['create_time_date'] < account.earliest_date):
            continue
