...
class RequestItemInfo:
    ...
    def run(self, account: Account) -> list[dict]:
        ...
        with self._progress_object() as progress:
            ...
            cursor = 0
            while True:
                if not (result := self._request_items_page_cursor_has_more(account.sec_user_id, cursor)):
                    break
                items_page, cursor, has_more = result
                items.extend(items_page)
                if not has_more or self._early_stop_by_date(account.earliest_date, cursor):
                    break
        return items
